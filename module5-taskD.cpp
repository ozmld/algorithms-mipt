...
class Treap {
 ...
 private:
  ...
  Treap LeftChild() const;
  Treap RightChild() const;
};

void Treap::Insert(Node* element) {
  std::pair<Treap, Treap> splitted = Split(element->key);
  root_ = Merge(splitted.first.Merge(splitted.first, element), splitted.second)
              .root_;
}
long long Treap::Kth(long long k) const {
  if (root_ == nullptr) {
    return -1;
  }
...
long long Treap::Next(long long x) const {
  if (root_ == nullptr) {
    return kInf;
  }
  if ((x + 1) == root_->key) {
...
long long Treap::Prev(long long x) const {
  if (root_ == nullptr) {
    return -1;
  }
  if ((x - 1) == root_->key) {
...
bool Treap::Find(long long x) const {
  if (root_ == nullptr) {
    return false;
  }
  if (root_->key == x) {
...
std::pair<Treap, Treap> Treap::Split(long long k) {
  if (root_ == nullptr) {
...
Treap Treap::LeftChild() const { return Treap(root_->l); }
Treap Treap::RightChild() const { return Treap(root_->r); }
